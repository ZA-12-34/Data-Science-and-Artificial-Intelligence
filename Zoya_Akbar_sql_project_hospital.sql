...
alter table hospitals
modify hospital_id int primary key;
...
alter table doctors
modify doctor_id int primary key;
...
insert into doctors (doctor_id, name, speciality, hospital_id, email)
values(
...
-- Basic Questions
-- Show the number of appointments in each month.
...
-- Show room numbers and their capacities in the Neurology department.
select name, room_no, capacity from departments, rooms
where name = 'Neurology';

-- Find the names, phones and appointment_dates of all patients with appointments in August.
select 
 p.name,
 p.phone,
 a.appointment_date
...
-- Find doctors who have not prescribed any medication yet.
...
-- Find the names of patients who have been prescribed more than one medication.
SELECT 
    p.name
FROM patients p
...
-- Adnvance Questions
-- Rank doctors by number of appointments (within their hospitals).
SELECT 
    d.hospital_id,
...
GROUP BY d.hospital_id, d.name
ORDER BY d.hospital_id, rank_in_hospital;

-- List the last 2 appointments for every patient.
SELECT 
...
SELECT 
    ...
    CASE 
        ...
        WHEN TIMESTAMPDIFF(YEAR, p.date_of_birth, CURDATE()) BETWEEN 19 AND 64 THEN 'Adult'
        ELSE 'Geriatric'
    END AS age_group
FROM appointments a
JOIN patients p 
...
-- Out of all the consultation appointments with cardiologists, break down how many are by pediatric, adult and geriatric patients each (age criteria is the same as above).
SELECT 
    CASE 
        WHEN TIMESTAMPDIFF(YEAR, p.date_of_birth, CURDATE()) <= 18 THEN 'Pediatric'
        WHEN TIMESTAMPDIFF(YEAR, p.date_of_birth, CURDATE()) BETWEEN 19 AND 64 THEN 'Adult'
...
WHERE a.reason LIKE '%Consultation%'
  AND d.speciality = 'Cardiology'     -- ✅ Corrected spelling
GROUP BY age_group
ORDER BY total_appointments DESC;

-- Every medication has a prescription frequency, i.e. the number of times it has been prescribed. Find all medications with the top three prescription frequencies (for example
-- if the top 3 frequencies are 8,7 and 6, then list all medications prescribed these number of times).
...
ranked AS (
    SELECT 
        ...
        name,
        frequency,
        DENSE_RANK() OVER (ORDER BY frequency DESC) AS freq_rank
...
JOIN hospitals h 
    ON dc.hospital_id = h.hospital_id
WHERE dc.doctor_count = (
    SELECT MIN(doctor_count)
    FROM doctor_counts
);

-- Out of the cardiology departments in each hospital, find out which hospital(s)'s cardiology department has the most number of rooms.
SELECT h.hospital_id, 
       h.name AS hospital_name,
       COUNT(r.room_id) AS cardiology_room_count
FROM hospitals h
...
HAVING COUNT(r.room_id) = (
    ...
    FROM (
        SELECT COUNT(r2.room_id) AS room_count
        FROM departments d2
        JOIN rooms r2 ON d2.department_id = r2.department_id
        WHERE d2.name = 'Cardiology'
        GROUP BY d2.hospital_id
    ) AS subquery
);

-- Find out the difference in days between the appointment dates of returning patients
SELECT 
    a.patient_id,
    DATEDIFF(MAX(a.appointment_date), MIN(a.appointment_date)) AS days_difference
FROM appointments a
